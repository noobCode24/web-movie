...
const HorizontalScollCard = ({data = [], heading, trending, media_type}) => {
  ...
  const handleNext = () => {
    contaierRef.current.scrollLeft += 1200
  }
  ...
  return (
    <div className='container mx-auto px-3 my-10'>
      <h2 className='text-xl scroll-beha lg:text-2xl font-bold mb-3 text-white capitalize'>{heading}</h2>
      ...
      <div className=' relative'>
        ...
        <div className='absolute top-0 hidden lg:flex justify-between w-full h-full items-center'>
          <button onClick={handlePrevious} className='bg-white p-1 text-black cursor-pointer rounded-full -ml-2 z-10'>
            <FaAngleLeft />
          </button>
          <button onClick={handleNext} className='bg-white p-1 text-black cursor-pointer rounded-full -mr-2 z-10'>
            <FaAngleRight />
          </button>
        </div>
      </div>
